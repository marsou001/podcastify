...
function EditPodcast({ podcastId }: { podcastId: Id<"podcasts"> }) {
  ...
  useEffect(() => {
    if (podcast === null || podcast === undefined) return;

    setImageURL(podcast.imageURL);
    setImagePrompt(podcast.imagePrompt ?? "");
    setImageStorageId(podcast.imageStorageId);
    
    setAudioURL(podcast.audioURL);
    setAudioStorageId(podcast.audioStorageId);
    setAudioDuration(0);

    setVoiceType(podcast.voiceType);
    setVoicePrompt(podcast.voicePrompt);
